 ...
 * Author: Mobax
 * Filename: REGISTERRequestHandler.java
...
public class REGISTERRequestHandler {
	private static final Logger LOGGER = MiTesterLog
			.getLogger(REGISTERRequestHandler.class.getName());

	/**
	 * Generating REGISTER Request
	 * 
	 ...
	 * @throws SipException
	 */
	public static Request createREGISTERRequest(String dialog)
			throws NullPointerException, java.text.ParseException,
			InvalidArgumentException, SipException {
		LOGGER.info("Generation of REGISTER Request is started");
		Request request;
		...
		if (dialog == null) {
			...
			SIPMessage sipMsg = ProcessSIPMessage.getSIPMessage(dialog,
					Request.REGISTER, SERVER_REQUEST);
		...
		CSeqHeader cSeqHeader = MessageHandlerHelper.createCSeqHeader(1,
				Request.REGISTER, headerFactory);
		...
		request = MessageHandlerHelper.createRequest(requestURI,
				Request.REGISTER, callid, cSeqHeader, fromHeader, toHeader,
				viaHeaders, maxForwards, messageFactoryImpl);
		LOGGER.info("Generation of REGISTER Request is ended");
		return request;
	}
