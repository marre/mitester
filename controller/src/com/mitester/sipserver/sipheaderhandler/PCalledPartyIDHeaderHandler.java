...
public class PCalledPartyIDHeaderHandler {
	...
	/**
	 * To remove PCalledPartyIDHeader from sip message
	 * @param header
	 * @param sipMessage
	 * @param type
	 * @return
	 ...
	 */
	public static SIPMessage removePCalledPartyIDHeader(Header header,
		...
		PCalledPartyIDHeader acceptEncoding = null;
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
			acceptEncoding = (PCalledPartyIDHeader) response
			        .getHeader(PCalledPartyIDHeader.NAME);
		else
			acceptEncoding = (PCalledPartyIDHeader) request
			        .getHeader(PCalledPartyIDHeader.NAME);
		...
		if (removeParams.size() == 0) {
			if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
				response.removeHeader(PCalledPartyIDHeader.NAME);
			else
				request.removeHeader(PCalledPartyIDHeader.NAME);
		}
