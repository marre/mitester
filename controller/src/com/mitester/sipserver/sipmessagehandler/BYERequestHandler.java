 ...
 * Author: Mobax
 * Filename: BYERequestHandler.java
...
public class BYERequestHandler {
	...
	public static Request createBYERequest(SIPMessage sipmsg,
	        boolean isOriginator) throws NullPointerException,
		...
		if (sipmsg != null) {
			if (isOriginator) {
				...
				FromHeader fromHeader = headerFactory.createFromHeader(from,
				        fromTag);
				ToHeader toHeader = headerFactory.createToHeader(to, toTag);
				request = messageFactoryImpl.createRequest(requestURI,
				        Request.BYE, sipmsg.getCallId(), cseq, fromHeader,
				        toHeader, viaHeaders, maxfwd);
				...
				ViaHeader viaHeaderBye = MessageHandlerHelper.createViaHeader(
				        LOOP_BACK_ADDRESS, 5070, "udp", branch, headerFactory);
				...
				CSeqHeader cseq = MessageHandlerHelper.createCSeqHeader(
				        invitecseq + 2, Request.BYE, headerFactory);
				...
				FromHeader fromHeader = MessageHandlerHelper.createFromHeader(
				        to, toTag, headerFactory);

				ToHeader toHeader = MessageHandlerHelper.createToHeader(from,
