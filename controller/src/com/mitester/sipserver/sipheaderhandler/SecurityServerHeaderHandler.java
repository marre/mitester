 ...
 * Author: Mobax
 * Filename: SecurityServerHeaderHandler.java
...
/**
 * SecurityServerHeaderHandler is used to create and remove ecurity-Server
 * header
 * 
 * RFC 3329 says,
 * 
 ...
 * 
 */
public class SecurityServerHeaderHandler {

	private static final Logger LOGGER = MiTesterLog
	        .getLogger(SecurityServerHeaderHandler.class.getName());

	/**
	 * addSecurityServerHeader is used to add Security-Server header
	 * 
	 * @param headerNew
	 * @param type
	 * @param sipMessage
	 * @return ecurity-Server header
	 * @throws SipException
	 * @throws ParseException
	 * @throws InvalidArgumentException
	 * @throws IndexOutOfBoundsException
	 */
	public static SecurityServerHeader addSecurityServerHeader(Header headerNew)
		...
		if (hvalue == null)
			hvalue = SECURITY;
		SecurityServerHeader header = hfimpl.createSecurityServerHeader();
		if (hvalue.length() != 0)
	...
	/**
	 * removeSecurityServerHeader is used to remove the ecurity-Server header
	 * 
	 * @param name
	 * @param removeParams
	 * @param sipMessage
	 ...
	 */
	public static SIPMessage removeSecurityServerHeader(Header header,
		...
		SecurityServerHeader from = null;
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
			from = (SecurityServerHeader) response
			        .getHeader(SecurityServerHeader.NAME);
		else
			from = (SecurityServerHeader) request
			        .getHeader(SecurityServerHeader.NAME);
		...
		if (removeParams.size() == 0) {
			if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
				response.removeHeader(SecurityServer.NAME);
			else
				request.removeHeader(SecurityServer.NAME);
		}
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
