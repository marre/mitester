...
public class SIPHeaderHandler {
	...
	public static SIPMessage addHeader(
		...
		for (com.mitester.jaxbparser.server.Header headerNew : sipHeaders) {
			...
			case ACCEPT_ENCODING: {
				...
				if (acceptEncoding == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						...
						copy = copyValues(SIPHeaders.getSipHeaderfromString(
								headerName).toString());
				}
				if (acceptEncoding != null) {
					LOGGER.info("Adding "
			...
			case CALL_ID: {
				...
				if (headerValue == null) {
					callid = CallIdHeaderHandler.createCallIdHeader(headerNew);
				}
				if (callid == null) {
					if (!headerValue.equals(COPY_VALUE)) {
			...
			case CSEQ: {
				...
				if (headerValue == null) {
					cSeqHeader = CSeqHeaderHandler.createCSeqHeader(headerNew);
				}
				if (cSeqHeader == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						if (headerValue.length() != 0) {
			...
			case FROM: {
				...
				if (headerValue == null) {
					fromHeader = FromHeaderHandler.createFromHeader(headerNew);
				}
				if (fromHeader == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						if (headerValue.length() != 0) {
			...
			case RACK: {
				...
				if (rack == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						if (headerValue.length() != 0) {
							boolean matchFound = validatingSIPHeaderValue("RA",
									headerValue);
							if (matchFound) {
			...
			case REASON: {
				...
				if (headerValue == null) {
					reason = ReasonHeaderHandler.createReasonHeader(headerNew);
				}
				if (reason == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						if (headerValue.length() != 0) {
			...
			case SERVER: {
				...
				if (headerValue == null) {
					server = ServerHeaderHandler.createServerHeader(headerNew);
				}
				if (server == null) {
					if (!headerValue.equals(COPY_VALUE)) {
			...
			case VIA: {
				...
				if (headerValue == null) {
					viaHeaders = ViaHeaderHandler.createViaHeader(headerNew);
				}
				if (viaHeaders == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						if (headerValue.length() != 0) {
			...
			case P_ACCESS_NETWORK_INFO: {
				...
				if (PAccessNetworkInfo == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						if (headerValue.length() != 0) {
							boolean matchFound = validatingSIPHeaderValue("P",
									headerValue);
							if (matchFound) {
								PAccessNetworkInfo = PAccessNetworkInfoHeaderHandler
										.createPAccessNetworkInfoHeader(headerNew);
			...
			case JOIN: {
				...
				if (headerValue == null) {
					joinHeader = JoinHeaderHandler.createJoinHeader(headerNew);
				}
				if (joinHeader == null) {
					if (!headerValue.equals(COPY_VALUE)) {
						if (headerValue.length() != 0) {
	...
	public static List<String> copyValues(String Headername)
		...
		LOGGER
				.info("Entered copied value from the Previous SIP Message with the Header name: "
						+ Headername);
		...
		while (headers.hasNext()) {
			...
			if (header.startsWith(Headername + ":")) {
				copy.add(header);
				LOGGER.info(Headername + " Header is found in the "
						+ ProcessSIPMessage.getMethodName(sipMsg)
						+ " SIP Message \n");
			} else {
				LOGGER.warn(Headername + " Header is not found in the "
						+ ProcessSIPMessage.getMethodName(sipMsg)
						+ " SIP Message \n");
			}
		}
