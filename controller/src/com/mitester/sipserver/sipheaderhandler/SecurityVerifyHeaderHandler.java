 ...
 * Author: Mobax
 * Filename: SecurityVerifyHeaderHandler.java
...
/**
 * SecurityVerifyHeaderHandler is used to create and remove Security-Verify
 * header
 * 
 * RFC 3329 says,
 * 
 ...
 * 
 */
public class SecurityVerifyHeaderHandler {

	private static final Logger LOGGER = MiTesterLog
	        .getLogger(SecurityVerifyHeaderHandler.class.getName());

	/**
	 * addSecurityVerifyHeader is used to add Security-Verify header
	 * 
	 * @param headerNew
	 * @param type
	 * @param sipMessage
	 ...
	 */
	public static SecurityVerifyHeader createSecurityVerifyHeader(
		...
		if (hvalue == null)
			hvalue = SECURITY;
		SecurityVerifyHeader header = hfimpl.createSecurityVerifyHeader();
		if (hvalue.length() != 0)
	...
	/**
	 * removeSecurityVerifyHeader is used to remove the Security-Verify header
	 * 
	 * @param name
	 * @param removeParams
	 * @param sipMessage
	 ...
	 */
	public static SIPMessage removeSecurityVerifyHeader(Header header,
		...
		SecurityVerifyHeader from = null;
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
			from = (SecurityVerifyHeader) response
			        .getHeader(SecurityVerifyHeader.NAME);
		else
			from = (SecurityVerifyHeader) request
			        .getHeader(SecurityVerifyHeader.NAME);
		...
		if (removeParams.size() == 0) {
			if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
				response.removeHeader(SecurityVerify.NAME);
			else
				request.removeHeader(SecurityVerify.NAME);
		}
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
