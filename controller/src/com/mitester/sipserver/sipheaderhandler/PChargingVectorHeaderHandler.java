...
public class PChargingVectorHeaderHandler {
	 ...
	 */
	public static SIPMessage removePChargingVectorHeader(Header header,
		...
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
			acceptEncoding = (PChargingVectorHeader) response
			        .getHeader(PChargingVectorHeader.NAME);
		else
			acceptEncoding = (PChargingVectorHeader) request
			        .getHeader(PChargingVectorHeader.NAME);
		...
		if (removeParams.size() == 0) {
			if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
				response.removeHeader(PChargingVectorHeader.NAME);
			else
				request.removeHeader(PChargingVectorHeader.NAME);
		}
