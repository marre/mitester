...
public class ReferToHeaderHandler {
	...
	public static SIPMessage removeReferToHeader(Header header,
		...
		ReferToHeader referto = null;
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
			referto = (ReferToHeader) response.getHeader(ReferToHeader.NAME);
		else 
			referto  = (ReferToHeader) request.getHeader(ReferToHeader.NAME);
		...
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
			...
			for (Param parameterName : removeParams) {
				referto.removeParameter(parameterName.getName());
			}
		}
		if (type.equalsIgnoreCase(SipServerConstants.SERVER_RESPONSE))
